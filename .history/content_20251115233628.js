...
function getExtensionURL(path) {
  try {
    // Try to get extension ID and base URL once
    if (!extensionBaseUrl && chrome && chrome.runtime) {
      try {
        ...
        if (chrome.runtime.getURL) {
          ...
          extensionBaseUrl = testUrl.substring(0, testUrl.lastIndexOf('/') + 1);
        }
      }
    }
    
    // If we have base URL, construct the full URL
    if (extensionBaseUrl) {
    ...
    // Context invalidated - return null but don't spam console
    if (extensionBaseUrl) {
      // Use cached URL even if context is invalidated
      return extensionBaseUrl + path;
    }
...
function createWitch() {
  ...
  chrome.storage.local.get(['selectedHat', 'selectedRobe'], function(result) {
    ...
    function dragStart(e) {
      ...
      if (!witchElement.contains(target) && target !== witchElement) {
        return;
      }
