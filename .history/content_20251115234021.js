...
// Initialize extension URL cache early
try {
  ...
  // Will try again in getExtensionURL
}
...
function getExtensionURL(path) {
  ...
  try {
    // Try to get extension ID and base URL
    if (chrome && chrome.runtime) {
      try {
        extensionId = chrome.runtime.id;
        extensionBaseUrl = `chrome-extension://${extensionId}/`;
        ...
        // Context might be invalidated, try direct call
    ...
    // Context invalidated - only log once to avoid spam
    if (!window.lwaContextErrorLogged) {
      console.warn('Little Witch Alcove: Extension context invalidated. Please reload the page to fix drag functionality.');
...
function createWitch() {
  ...
  chrome.storage.local.get(['selectedHat', 'selectedRobe'], function(result) {
    ...
    function dragStart(e) {
      ...
      // More permissive check - if target is witchElement or any child, allow drag
      // Since container and images have pointer-events: none, clicks pass through to witchElement
      // But we still check if the click originated from within the witch
      const isWitchElement = target === witchElement;
      const isWitchChild = witchElement && witchElement.contains(target);
      
      if (!isWitchElement && !isWitchChild) {
