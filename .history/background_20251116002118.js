...
// Import productivity checker
importScripts('productivity-checker.js');

// Cache to avoid checking the same tab multiple times quickly
const productivityCache = new Map();
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes

// Track API failures to disable monitoring if API is broken
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'startTimer') {
    ...
    chrome.storage.local.set({
      ...
      lastAwardedMinute: 0,
      productivityMonitoring: true, // Enable productivity monitoring
      hadUnproductivePause: false
    });
    
    // Set up alarm to award potions every minute
    chrome.alarms.create('awardPotion', { periodInMinutes: 1 });
    
    // Start monitoring productivity every 30 seconds
    ...
    chrome.storage.local.get(['timerPaused', 'timerPausedTime', 'timerTotalPausedTime'], (result) => {
      if (!result.timerPaused) {
        // Pausing now - record the pause time (manual pause, not due to unproductive tab)
