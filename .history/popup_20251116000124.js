...
function loadUnlockedItems() {
  chrome.storage.local.get(['unlockedItems'], function(result) {
    const unlockedItems = result.unlockedItems || { hat: [1], robe: [1] }; // Default: only first items unlocked
...
document.querySelector('.modal-overlay').addEventListener('click', hidePurchaseModal);
