...
function createWitch() {
  ...
  chrome.storage.local.get(['selectedHat', 'selectedRobe'], function(result) {
    ...
    const witchWidth = 50; // Smaller size
    const initialX = (window.innerWidth / 2) - (witchWidth / 2);
    ...
    let floorY = window.innerHeight - witchWidth; // Initial estimate
    ...
    function updateFloorPosition() {
      if (witchBodyImg && witchBodyImg.complete) {
        const imageHeight = witchBodyImg.naturalHeight || 50;
        const imageWidth = witchBodyImg.naturalWidth || 50;
        // Calculate actual displayed height (maintaining aspect ratio)
        const displayedHeight = (imageHeight / imageWidth) * witchWidth;
  ...
  function updatePhysics() {
    ...
    if (currentX < 0) {
      currentX = 0;
      velocityX = 0;
    } else if (currentX > window.innerWidth - witchWidth) {
      currentX = window.innerWidth - witchWidth;
      velocityX = 0;
    }
