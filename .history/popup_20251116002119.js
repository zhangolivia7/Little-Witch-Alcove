...
function stopTimer() {
  ...
  chrome.storage.local.get(['timerStartTime', 'timerTotalPausedTime', 'hadUnproductivePause'], (result) => {
    if (result.timerStartTime) {
      ...
      // Celebrate if session was over 1 minute and had no unproductive pauses
...
// Resume button removed - timer now auto-resumes when returning to productive tab
...
function triggerCelebration() {
  ...
  const colors = ['#CAAFE5', '#b89dd4', '#937640', '#ffffff', '#ffd700'];
...
function createParticle(container, colors) {
  ...
  const animName = `particleFloat${Date.now()}_${Math.random()}`;
...
function createSparkle(container) {
  ...
  const animName = `sparkleFloat${Date.now()}_${Math.random()}`;
  
  // Create keyframes for this sparkle
  const style = document.createElement('style');
  style.textContent = `
    @keyframes ${animName} {
      0% {
      ...
      50% {
        transform: translate(${deltaX}px, ${deltaY}px) scale(1) rotate(180deg);
        opacity: 1;
      }
      100% {
  ...
  sparkle.style.cssText = `
    ...
    pointer-events: none;
    z-index: 10000;
    box-shadow: 0 0 10px #CAAFE5, 0 0 20px #CAAFE5;
