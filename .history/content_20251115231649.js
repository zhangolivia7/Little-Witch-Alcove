...
function createWitch() {
  ...
  chrome.storage.local.get(['selectedHat', 'selectedRobe'], function(result) {
    ...
    function dragStart(e) {
      // Check if click is on the witch (including all child elements)
      const target = e.target;
      const clickedOnWitch = target === witchElement || 
                            target === witchContainer ||
                            target === witchBodyImg ||
                            target === witchHeadImg ||
                            (witchElement && witchElement.contains(target));
      
      if (!clickedOnWitch) return;
      ...
      if (witchContainer) witchContainer.style.cursor = 'grabbing';
    ...
    function dragEnd(e) {
      ...
      if (witchContainer) witchContainer.style.cursor = 'grab';
      
      // Don't give velocity - let physics handle it naturally
      // Only set isOnFloor based on current position
      isOnFloor = currentY >= floorY - 1; // Allow 1px tolerance
  ...
  // Mouse events - attach to both element and container
  witchElement.addEventListener('mousedown', dragStart);
  if (witchContainer) {
    witchContainer.addEventListener('mousedown', dragStart);
  }
  document.addEventListener('mousemove', drag);
  document.addEventListener('mouseup', dragEnd);

  // Touch events - attach to both element and container
  witchElement.addEventListener('touchstart', dragStart);
  if (witchContainer) {
    witchContainer.addEventListener('touchstart', dragStart);
  }
